...
def _process_and_create_dataset(config: Config, image_dir: str, csv_path: str, label_map_path: str, train_npy_path: str, test_npy_path: str):
    ...
    # 이 단계에서 live_test.py가 사용할 JSON 파일을 생성하고, 메모리상에서도 라벨 맵을 반환.
...
def main():
    ...
    if args.mode == 'train':
        ...
        incremental_label_map = _process_and_create_dataset(
            config,
        ...
        # 기존(basic) 데이터의 라벨 맵은 파일에서 직접 로드
        try:
            with open(config.LABEL_MAP_PATHS['basic'], 'r') as f:
                basic_label_map = json.load(f)
            logging.info(f"----- 기존 라벨 맵 로드 완료: {config.LABEL_MAP_PATHS['basic']}")
        except FileNotFoundError:
            logging.error(f"----- 기존 라벨 맵({config.LABEL_MAP_PATHS['basic']})을 찾을 수 없습니다. 'train' 모드를 먼저 실행하세요.")
            sys.exit(1)
        ...
        inc_train_grouped = data_manager.load_data_grouped_by_label(config.INCREMENTAL_TRAIN_DATA_PATH, incremental_label_map)
        basic_train_grouped = data_manager.load_data_grouped_by_label(config.BASIC_TRAIN_DATA_PATH, basic_label_map)
        basic_test_grouped = data_manager.load_data_grouped_by_label(config.BASIC_TEST_DATA_PATH, basic_label_map)
        ...
        data_manager.combine_and_save_data(combined_map=combined_label_map, basic_map=basic_label_map, incremental_map=incremental_label_map)
