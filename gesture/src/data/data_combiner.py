...
class DataCombiner:
    ...
    NPY 파일 로드, 데이터셋 결합 및 저장, 라벨별 데이터 그룹화 기능을 제공합니다.
    """

    def __init__(self, config: FileConfig):
        """
        DataCombiner를 초기화합니다.
    ...
    def load_and_combine_all_datasets(self, combined_map: Dict[str, int], basic_map: Dict[str, int], incremental_map: Dict[str, int]) -> Tuple[np.ndarray, np.ndarray] | Tuple[None, None]:
        """
        기본 및 증분 데이터셋을 로드하고 통합 라벨 맵에 따라 결합합니다.
        ...
        all_y_numeric: List[int] = [] # Store numeric labels temporarily
        ...
        # Convert numeric labels back to string labels for saving to NPY
        ...
        return np.array(all_X), all_y_string # Return X as float, y as string

    def save_combined_dataset(self, X: np.ndarray, y: np.ndarray):
        """
        결합된 데이터셋을 NPY 파일로 저장합니다.
    ...
    def load_data_grouped_by_label(self, npy_path: str) -> Dict[str, np.ndarray]:
        """
        NPY 파일에서 데이터를 로드하고, 라벨별로 그룹화합니다.
        ...
        """
        X, y_string_labels_from_npy = self.converter._load_npy_data(npy_path) # y_string_labels_from_npy are already strings
        if X is None:
            return {}

        # No need for index_to_label_map or further conversion here
        # as y_string_labels_from_npy already contains the desired string labels.

        grouped_data: Dict[str, List[np.ndarray]] = {}
        for i, str_label in enumerate(y_string_labels_from_npy): # Iterate directly over string labels
    ...
    def combine_and_save_data(self, combined_map: Dict[str, int], basic_map: Dict[str, int], incremental_map: Dict[str, int]):
        """
        기존 및 증분 데이터셋을 결합하고, 최종 NPY 파일을 저장합니다.
