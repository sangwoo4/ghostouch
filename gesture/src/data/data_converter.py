...
class DataConverter:
    ...
    CSV에서 NPY 데이터셋을 생성하거나, NPY 데이터를 로드하는 기능을 제공합니다.
    """

    def create_npy_dataset(self, csv_path: str, npy_path: str):
        """
        CSV 파일로부터 NPY 데이터셋을 생성합니다.
        ...
        Raises:
            FileNotFoundError: CSV 파일을 찾을 수 없을 때 발생합니다.
        """
        ...
        X = df.drop(columns=['label']).values.astype(np.float32) # Ensure X is float32
        y_string_labels = df['label'].values # Directly use the string labels from CSV

        # Stack X (float) with y_string_labels (string). This will result in an object array.
