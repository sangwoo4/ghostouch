<img src="https://capsule-render.vercel.app/api?type=rounded&height=300&color=gradient&text=Ghostouch&desc=2025%20오픈소스%20개발자%20대회%20TEAM%20Pentagon&descAlign=50&descAlignY=65" width="100%"/>
...
![AVFoundation](https://img.shields.io/badge/AVFoundation-5A5A5A?style=for-the-badge&logo=apple&logoColor=white)
...
<details>
  ...
  1. **실시간 제스처 인식**  
      - 전면 카메라와 Mediapipe Hand Detection을 활용하여 손 모양과 움직임을 빠르게 인식
  
  2. **사용자 지정 제스처 매핑**  
      - 원하는 제스처(엄지척)를 특정 기능(앱 실행)에 직접 연결 가능
  
  3. **백그라운드 실행**  
      - 앱을 최소화해도 제스처 인식이 계속 동작하며, 실행 시간을 원하는 대로 설정
  
  4. **배터리 절약**  
      - 필요할 때만 동작하도록 설정해 불필요한 자원 사용을 줄이고 배터리를 효율적으로 관리
  
  5. **온디바이스 처리**  
      - 모든 연산을 스마트폰 내에서 처리해 인터넷 연결이 필요 없으며, 개인정보도 안전하게 보호
  
  6. **증분 학습 지원**  
      - 기존 제스처와 새 제스처를 함께 학습할 수 있고, 새로운 사용자 정의 제스처를 자유롭게 추가 가능
  
  <a id="시스템-아키텍처"></a>
  ### 🖥️ 시스템 아키텍처
  
  <p align="center">
    <img src="https://github.com/user-attachments/assets/83101da1-fc27-413b-8bdb-eca46e6480e5" width="80%" alt="System Architecture"/>
  </p>
